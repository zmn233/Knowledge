 ...
 */
 ...
 util.jsonp = function (url, onsuccess, onerror, charset) {
     ...
     script.onload = script.onreadystatechange = function () {
      ...
      // 4.if(!this.readyState||this.readyState=='loaded'||this.readyState=='complete')，!this.readyState表示不是IE11以下浏览器（IE11以下浏览器也是支持onreadystatechange事件的）
   ...
   error = () => {}
     ...
     if (url) {
         ...
         if (type === 'GET') {
             ...
             // 对 url 进行替换是解决data为空对象导致url后面多一个 ? 的现象，第三个参数表示是否为异步请求，默认为true
...
function my_ajax(
  ...
  if(url) {
    ...
    xhr.onreadystatechange = function() {
      if(xhr.readyStatus == 4 && (xhr.status >= 200 && xhr.status < 300 || xhr.status == 304)) {
